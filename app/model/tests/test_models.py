...
class MyTestCase(unittest.TestCase):
    def test_audio_weights_exist(self):
        weights = Path("app/model/weights/audio/finetuned_Full-RESNET_model.pt")
        self.assertTrue(True, os.path.isfile(weights))

    def test_lyrics_weights_exist(self):
        weights = Path("app/model/weights/lyrics/finetuned_BERT_model-epoch-v.3.pt")
        self.assertTrue(True, os.path.isfile(weights))
    ...
    def test_audio_preprocessing(self):
        image_file = Path("app/model/tests/test_files/532171.png")
        self.assertEqual(torch.Tensor, type(audio_preprocessor(image_file)))

    def test_lyrics_encoding(self):
        lyrics_file = Path("app/model/tests/test_files/532171.txt")
